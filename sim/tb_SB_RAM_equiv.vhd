...
entity tb_SB_RAM_equiv is
end tb_SB_RAM_equiv;

architecture sim of tb_SB_RAM_equiv is
...
begin
	...
	D1 : entity orig.SB_RAM port map (address=>address, clock=>clk, data=>data, wren=>wren, q=>q_orig);
	D2 : entity work.SB_RAM port map (address=>address, clock=>clk, data=>data, wren=>wren, q=>q_port);
	...
	begin
		...
		for a in 0 to 127 loop
			address <= std_logic_vector(to_unsigned(a, 7));
		...
		for a in 0 to 127 loop
			address <= std_logic_vector(to_unsigned(a, 7)); tick;
		end loop;
		...
		for i in 0 to 1999 loop
			...
			address <= std_logic_vector(resize(seed(6 downto 0), 7));
			data <= std_logic_vector(seed(15 downto 8));
			wren <= seed(3); tick;
		end loop;
		wren <= '0'; for i in 0 to 3 loop tick; end loop;
		checking <= false; wait for 1 ns;
		report "SB_RAM : comparaisons=" & integer'image(compares) & "  divergences=" & integer'image(mismatch);
		if mismatch = 0 and compares > 1000 then
			report "SB_RAM : EQUIVALENTES" severity note;
		else
			report "SB_RAM : ECHEC (divergences ou trop peu de comparaisons)" severity failure;
		end if;
		running <= false; wait;
	end process;
end sim;
