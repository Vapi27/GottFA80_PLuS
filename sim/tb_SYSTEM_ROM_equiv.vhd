...
entity tb_SYSTEM_ROM_equiv is
end tb_SYSTEM_ROM_equiv;

architecture sim of tb_SYSTEM_ROM_equiv is
...
begin
	...
	D1 : entity orig.SYSTEM_ROM port map (address=>address, clock=>clk, data=>data, wren=>wren, q=>q_orig);
	D2 : entity work.SYSTEM_ROM port map (address=>address, clock=>clk, data=>data, wren=>wren, q=>q_port);
	...
	begin
		...
		for a in 0 to 8191 loop
			address <= std_logic_vector(to_unsigned(a, 13));
		...
		for a in 0 to 8191 loop
			address <= std_logic_vector(to_unsigned(a, 13)); tick;
		end loop;
		...
		for i in 0 to 1999 loop
			...
			address <= std_logic_vector(resize(seed(12 downto 0), 13));
			data <= std_logic_vector(seed(15 downto 8));
			wren <= seed(3); tick;
		end loop;
		wren <= '0'; for i in 0 to 3 loop tick; end loop;
		checking <= false; wait for 1 ns;
		report "SYSTEM_ROM : comparaisons=" & integer'image(compares) & "  divergences=" & integer'image(mismatch);
		if mismatch = 0 and compares > 1000 then
			report "SYSTEM_ROM : EQUIVALENTES" severity note;
		else
			report "SYSTEM_ROM : ECHEC (divergences ou trop peu de comparaisons)" severity failure;
		end if;
		running <= false; wait;
	end process;
end sim;
