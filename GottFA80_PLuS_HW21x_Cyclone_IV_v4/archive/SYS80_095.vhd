...
port map(
   ...
   show  => not game_running,
	...
	-- input (display data)
	--display1	=> ( x"F",x"F",x"F",x"F",x"1",x"1",x"0" ), -- 110 shouldnt it 094?
	display1	=> ( x"F",x"F",x"F",x"F",x"0",x"9",x"5" ),
	display2	=> ( x"F",x"F",x"F",x"F",dig2, dig1, dig0),
	display3	=> ( x"F",x"0",x"5",x"0",x"9",x"6",x"3" ),
	display4	=> ( x"F",x"F",x"F",x"F",x"F",x"F",x"F" ),
	status_d	=> ( x"F",x"F",x"F",x"F",x"F",x"F",x"F" )
	);
...
-- zero in case of
disp_segments <= 
	bm_segments when game_running = '0' else  
