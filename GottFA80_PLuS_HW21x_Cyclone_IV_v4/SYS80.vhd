...
--
-- for FPGA v3 changes: shared SPI; no Serial
-- v510 inital version for HW 2.1x EP4CE6F17C8
-- v511 output ports corrected for sound/audio_rx
...
entity SYS80 is
	generic(
		...
		-- the sound_link UART feeds the ESP (sndmode=hybrid, plays speech + complex-80B). Off by default.
...
port map(
   ...
   show  => not game_running,
	...
	-- input (display data)		
	display1	=> "    511",  -- SW VERSION TO BE PUT HERE
...
port map(
	...
	o_ta_start => open, o_ta_decay => open,           -- (Cyclone IV variant: tournament integration TODO)
...
-- HYBRID build: GOSOF80 stays the sound source (drives Sound/PIN_7 + DFP_tx/PIN_2) and the
-- sound_link UART feeds the ESP on the Debug pin (speech + complex-80B). Only Debug is driven
-- here. `and not esp_sound` guards a both-true misconfig.
